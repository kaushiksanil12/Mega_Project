...
def main():
    ...
    english_audio_folder = "/app/final_chunks"
    ...
    for hindi_text_path in hindi_files:
        ...
        voice_to_clone_path = os.path.join(english_audio_folder, f"{base_filename}.wav")
        if not os.path.exists(voice_to_clone_path):
            print(f"❌ ERROR: Cannot find original voice file: {voice_to_clone_path}. Skipping.")
            continue
